...
# Set the number of times to repeat the command
$numRepeats = 10

# Use a for loop to repeat the command
for ($i = 0; $i -lt $numRepeats; $i++) {
  # Calculate the ceiling of 100 divided by 2
  $k = [Math]::Ceiling(100/2)

  # Create a new WScript.Shell object
  $o = New-Object -ComObject WScript.Shell

  # Use a for loop to send the [char] 175 keystroke $k times
  for($i = 0; $i -lt $k; $i++) {
    $o.SendKeys([char] 175)
  }

  # Wait for 2 seconds before continuing with the next repetition
